...
module.exports = {
    ...
    extractInfo: function (config) {
        ...
        var widgetInfo = {},
            ...
            widgetDescription = config.widget.children.description.validationResult,
            widgetName = config.widget.children.name.validationResult,
            ...
            permissions = config.rawJSON.widget[0]["rim:permissions"] &&
                          config.rawJSON.widget[0]["rim:permissions"][0]["rim:permit"] || [],
        ...
        widgetInfo.invokeTargets = config.rawJSON.widget[0]["rim:invoke-target"];
        ...
        widgetInfo.description = widgetDescription.reduce(function (description, item) {
            if (item.valid) {
                description[item.attributes["xml:lang"].value || "default"] = item.value;
            }
            return description;
        }, {});

        widgetInfo.name = widgetName.reduce(function (name, item) {
            if (item.valid) {
                name[item.attributes["xml:lang"].value || "default"] = item.value;
            }
            return name;
        }, {});

        widgetInfo.permissions = permissions.map(function (item) {
            return item["#text"][0];
        });
                ...
                "rim:permissions": {
                    ...
                    children: {
                        "rim:permit": {
                            ...
                            occurrence: 0,
                            type: "list",
                            listValues: ["access_shared", "record_audio", "read_geolocation", "read_device_identifying_information", "use_camera"]
                        }
                    }
                },
                "rim:navigation": {
