...
_self = {
    initialize: function () {
        _motion = db.retrieveObject(_SAVE_KEY) || _motion;
    },
    ...
    setInfo: function (e) {
        ...
        else if (e.acceleration && e.accelerationIncludingGravity) {
            if (e.orientation) {
                dt = (new Date().getTime() - _motion.timestamp) / 1000;
                dalpha = (e.orientation.alpha - _motion.orientation.alpha) / dt;
                dbeta = (e.orientation.beta - _motion.orientation.beta) / dt;
                dgamma = (e.orientation.gamma - _motion.orientation.gamma) / dt;

                _motion = {
                    acceleration: new Acceleration(e.acceleration.x, e.acceleration.y, e.acceleration.z),
                    accelerationIncludingGravity: new Acceleration(e.accelerationIncludingGravity.x, e.accelerationIncludingGravity.y, e.accelerationIncludingGravity.z),
                    orientation: new Rotation(e.orientation.alpha, e.orientation.beta, e.orientation.gamma),
                    rotationRate: new Rotation(dalpha, dbeta, dgamma),
                    timestamp: new Date().getTime()
                };
        ...
        db.saveObject(_SAVE_KEY, _motion);
