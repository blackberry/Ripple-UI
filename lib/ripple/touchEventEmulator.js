...
function _marshalEvents(frame) {
    utils.forEach(["mousedown", "mousemove", "mouseup"],
        function (event) {
            frame.contentDocument.addEventListener(event, _raiseTouchEvent, true);
        });

    frame.contentWindow.Node.prototype.ontouchstart = null;
    frame.contentWindow.Node.prototype.ontouchend = null;
    frame.contentWindow.Node.prototype.ontouchmove = null;
}

self = module.exports = {
    mask: function (frame) {
        frame.contentWindow.addEventListener("DOMContentLoaded", function () {
            _marshalEvents(frame);
        });
    }
};
