...
self = module.exports = {
    ...
    appLocation: function () {
        if (require('ripple/ui').registered("omnibar")) {
            var path = document.querySelector(".omni-bar input").value;
            if (path !== "about:blank" && path !== "") {
                if (path.lastIndexOf("/") > 7) { // http https or file
                    if (path.substring(path.lastIndexOf("/")).indexOf(".") !== -1) {
