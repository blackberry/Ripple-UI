...
function _objectFactory(parentObj, objects, allowed) {
    utils.forEach(objects, function (obj, key) {
        var constructedObject = obj.path ? require('ripple/platform/' + obj.path) : {};

        if (allowed(obj)) {
            parentObj[key] = constructedObject;

            if (obj.children) {
                _objectFactory(parentObj[key], obj.children, allowed);
            }
        }
    });
}
