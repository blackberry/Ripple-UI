...
event.on("HardwareKeyDefault", function (key) {
    if (key === 0 || key === "0") { //back button key
        require('ripple/client/emulatorBridge').window().history.back();
    }
});
...
_self = module.exports = {
    initialize: function () {
        _devices = [
            ...
            hash[deviceID] = require('ripple/client/devices/' + deviceID);
            return hash;
        }, {});

        var current = this.getCurrentDevice();
        require('ripple/client/bus').send('userAgent', current.userAgent);
    },
