...
var db = require('ripple/client/db'),
    ...
    tooltip = require('ripple/client/ui/plugins/tooltip'),
                ...
                case "select":
                    if (opts.options) {
                        ...
                        opts.options.map(function (option) {
                            var settings;
                            input += "<option" + (option.selected ? " selected" : "") + ">" + option.value + "</option>";
                            if (option.selected) {
                                settings = _settings.get();
                                settings[item] = option.value;
...
function create() {
    ...
    case "webworks.bb10":
        ...
        launch.row("deployTarget", "Target", {
            type: "select",
            options : [
                {value: "Simulator", selected: true},
                {value: "Device"}
            ]
        });
